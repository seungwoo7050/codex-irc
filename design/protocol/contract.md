...
- v0.9.0에서는 INI 설정 파일로 서버명을 포함한 운영 파라미터를 지정하고, REHASH/SIGHUP으로 런타임 리로드 규칙을 고정하며, per-connection 레이트리밋과 백프레셔 정책을 동결한다.
...
- 지원 섹션 및 키(기본값 포함):
  ...
  - `[limits]`
    - `messages_per_5s` (기본: `0`, 0이면 비활성화) — 5초 윈도우 동안 허용되는 PRIVMSG/NOTICE 전송 횟수 상한. 0이면 레이트리밋을 적용하지 않는다.
    - `outbound_lines` (기본: `16`) — 송신 큐 상한(라인 수). 0 또는 누락 시 기본값을 사용한다.
...
- 상한: **16개 라인**(5초 윈도우 내 큐잉 기준).
- 새 라인을 추가하려 할 때 큐가 가득 차면 **큐 주인 클라이언트를 로그에 남기고 즉시 종료**한다. 초과한 라인은 전송하지 않는다.

## 레이트리밋 (v0.9.0)
- 적용 대상: 등록된 클라이언트가 발신하는 PRIVMSG/NOTICE.
- 파라미터: `[limits] messages_per_5s` 값을 최대 횟수로 사용하며, 윈도우는 고정 5초이다. 값이 0이면 레이트리밋을 비활성화한다.
- 슬라이딩 윈도우: 각 클라이언트는 최근 5초간 보낸 PRIVMSG/NOTICE 개수를 추적하며, 윈도우 내 누적이 상한을 넘으면 위반으로 간주한다.
- 위반 시 정책:
...
### v0.9.0에서 사용하는 numeric 추가 목록
- 439 ERR_RATEEXCEEDED – 레이트리밋 위반 알림
